...
if user == '' or passw == '':
    ...
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('[COLOR=FF67cc33]mafiatv[/COLOR]', 'Please set your Trakt.tv credentials','or register if you don have an account','','Cancel','Login')
    if ret == 1:
...
def trackShow(title,track):
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('[COLOR=FF67cc33]mafiatv[/COLOR]', 'Track '+title,'Do you want to track '+title+'?','','NO','YES')
    if ret == 1:
        import hashlib,json
...
def untrackShow(title,track):
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('[COLOR=FF67cc33]mafiatv[/COLOR]', 'Untrack '+title,'Do you want to untrack '+title+'?','','NO','YES')
    if ret == 1:
        import hashlib,json
        data = {'username':user,'password':hashlib.sha1(passw).hexdigest(),'shows':[{'tvdb_id': int(track)}]}
