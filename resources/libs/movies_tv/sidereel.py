...
def downloadFile(url,dest):
    try:
        ...
        main.ErrorReport(e)
        dialog.ok("MafiaTV", "Report the error below at " + main.supportsite, str(e), "We will try our best to help you")
...
if user == '' or passw == '':
    ...
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('[COLOR=FF67cc33]mafiatv[/COLOR]', 'Please set your SideReel credentials','or register if you don have an account','','Cancel','Login')
    if ret == 0:
        """
        ...
        if (keyb.isConfirmed()):
            ...
            if len(keyb.getText())<5:
                dialog.ok("[COLOR=FF67cc33]mafiatv[/COLOR]", "Username must be at least 5 characters", "")
            if (keyb.isConfirmed()) and len(keyb.getText())>=5:
                ...
                if len(keyb.getText())<6:
                    dialog.ok("[COLOR=FF67cc33]mafiatv[/COLOR]", "Password must be at least 6 characters", "")
...
def UNTRACKSHOW(track):
    ...
    if check:
        ...
        if match2:
            sitetoken=match2[0]
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('MafiaTV', 'Are you sure you want to stop tracking show?','','','No', 'Yes')
        if ret == 1:
            cookie = open(cookie_file).read()
            user=re.findall('SrLoggedIn=(.+?);',cookie)[0]
            method=re.findall('SrLoginMethod=(.+?);',cookie)[0]
