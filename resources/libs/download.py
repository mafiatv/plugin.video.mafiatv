...
def download(url, dest, title=None, referrer=None):
    if not title:
        title  = 'XBMC Download'
    ...
    cmd    = 'RunScript(%s, %s, %s, %s, %s)' % (script, url, dest, title, referrer)
...
if __name__ == '__main__': 
    if 'download.py' in sys.argv[0]:       
        doDownload(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
