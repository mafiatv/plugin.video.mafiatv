...
fav = False
hostlist = None
Dir = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.mafiaTV', ''))
repopath = xbmc.translatePath(os.path.join('special://home/addons/repository.mash2k3', ''))
datapath = xbmc.translatePath(selfAddon.getAddonInfo('profile'))
supportsite = 'mafiatvxbmc.com'
...
if selfAddon.getSetting("skin") == "0":
    art = 'https://raw.github.com/mafiatv/MafiaTVArtwork/master/skins/vector'
    fanartimage=Dir+'fanart2.jpg'
else:
    art = 'https://raw.github.com/mafiatv/MafiaTVArtwork/master/skins/greenmonster'
    fanartimage=Dir+'fanart.jpg'
elogo = xbmc.translatePath('special://home/addons/plugin.video.mafiaTV/resources/art/bigx.png')
slogo = xbmc.translatePath('special://home/addons/plugin.video.mafiaTV/resources/art/smallicon.png')
...
def ErrorReport(e):
    elogo = xbmc.translatePath('special://home/addons/plugin.video.mafiaTV/resources/art/bigx.png')
    xbmc.executebuiltin("XBMC.Notification([COLOR=FF67cc33]MafiaTV Error[/COLOR],"+str(e)+",10000,"+elogo+")")
    xbmc.log('***********MafiaTV Error: '+str(e)+'**************', xbmc.LOGERROR)
...
#             print status,
        ...
        if not silent:
            ...
            dialog.ok("MafiaTV", "Report the error below at " + supportsite, str(e), "We will try our best to help you")
        return False
...
def CheckVersion():
    try:
        link=OPENURL('http://repo.mafiatvxbmc.com/plugin.video.mafiaTV/resources/libs/main.py',verbose=False)
    except:
        link='nill'
    ...
    if len(match)>0:
        if VERSION != str(match[0]):
                dialog = xbmcgui.Dialog()
                ok=dialog.ok('[B]New Update Available![/B]', "Your version of MafiaTV is outdated." ,'The current available version is '+str(match[0]),'To update goto addons under system settings')
                print 'MafiaTV v'+VERSION+' is Outdated'
                return False
        else:
            print 'MafiaTV v'+VERSION+' is Up to Date'
            return True
...
def checkGA():
    if selfAddon.getSetting("gastatus") == "true":
        ...
        print "mafiatv Google Analytics disabled"
...
def APP_LAUNCH():
        ...
        for build, PLATFORM in match:
            ...
            utm_track = utm_gif_location + "?" + \
                    ...
                    "&utmn=" + str(randint(0, 0x7fffffff)) + \
                    "&utmt=" + "event" + \
                    "&utme="+ quote("5(APP LAUNCH*"+"MafiaTV v"+VERSION+"/ Repo v"+RepoVer+"*"+build+"*"+PLATFORM+")")+\
                    "&utmp=" + quote(PATH) + \
                    "&utmac=" + UATRACK + \
                    "&utmcc=__utma=%s" % ".".join(["1", VISITOR, VISITOR, VISITOR,VISITOR,"2"])
            try:
            ...
            utm_track = utm_gif_location + "?" + \
                    ...
                    "&utmn=" + str(randint(0, 0x7fffffff)) + \
                    "&utmt=" + "event" + \
                    "&utme="+ quote("5(APP LAUNCH*"+"MafiaTV v"+VERSION+"/ Repo v"+RepoVer+"*"+PLATFORM+")")+\
                    "&utmp=" + quote(PATH) + \
                    "&utmac=" + UATRACK + \
                    "&utmcc=__utma=%s" % ".".join(["1", VISITOR, VISITOR, VISITOR,VISITOR,"2"])
            try:
...
def addDirX(name,url,mode,iconimage,plot='',fanart='',dur=0,genre='',year='',imdb='',tmdb='',isFolder=True,searchMeta=False,addToFavs=True,
    ...
    Commands.append(('[B][COLOR=FF67cc33]mafiatv[/COLOR] Settings[/B]','XBMC.RunScript('+xbmc.translatePath(mashpath + '/resources/libs/settings.py')+')'))
    if menuItemPos != None:
