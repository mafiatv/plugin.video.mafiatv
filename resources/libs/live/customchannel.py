...
def XmlIns():
        dir = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.mafiaTV/resources/message', ''))
        chlg = os.path.join(dir, 'xmlchannels.txt')
        main.TextBoxes("[B][COLOR red]mafiatv XML Instructions![/B][/COLOR]",chlg)

def addPlaylist(folder):
    dialog = xbmcgui.Dialog()
    ret = dialog.select('[COLOR=FF67cc33][B]Choose Entry Type[/COLOR][/B]',['[B][COLOR=FF67cc33]Select a mafiatv XML Using Filemanager[/COLOR][/B]','[B][COLOR=FF67cc33]Select a mafiatv XML Using Set User Source[/COLOR][/B]','[B][COLOR=FF67cc33]Enter a mafiatv XML URL[/COLOR][/B]','[B][COLOR blue]Select a Livestreams XML Using Filemanager[/COLOR][/B]','[B][COLOR blue]Select a Livestreams XML Using Set User Source[/COLOR][/B]','[B][COLOR blue]Enter a Livestreams XML URL[/COLOR][/B]'])
    if ret == -1:
        return
    else:
        if ret == 0:
...
def removePlaylist(title,murl,folders):
    if os.path.exists(PlaylistFile):
        ...
        else: xbmc.executebuiltin("XBMC.Notification([B][COLOR green]MafiaTV[/COLOR][/B],[B]You Have No Playlists[/B],1000,"")")
    return
                    ...
                    if os.path.exists(PlaylistFile):
                        ...
                        for name,url,fanart,folder,typeXml,thumb in reversed (playlist):
                            if title == urllib.unquote_plus(name) and urllib.unquote_plus(folders)==urllib.unquote_plus(folder):
                                ...
                                xbmc.executebuiltin("Container.Refresh")
        if ret == 1:
            retb = dialog.select('[COLOR=FF67cc33][B]Choose Entry Type[/COLOR][/B]',['[B][COLOR=FF67cc33]Select a mafiatv XML Using Filemanager[/COLOR][/B]','[B][COLOR=FF67cc33]Select a mafiatv XML Using Set User Source[/COLOR][/B]','[B][COLOR=FF67cc33]Enter a mafiatv XML URL[/COLOR][/B]','[B][COLOR blue]Select a Livestreams XML Using Filemanager[/COLOR][/B]','[B][COLOR blue]Select a Livestreams XML Using Set User Source[/COLOR][/B]','[B][COLOR blue]Enter a Livestreams XML URL[/COLOR][/B]'])
            if ret == -1:
                return
            else:
                if retb == 0:
                                ...
                                if title == urllib.unquote_plus(name) and urllib.unquote_plus(folders)==urllib.unquote_plus(folder):
                                    ...
                                    for name,url,fanart,folder,typeXml,thumb in playlist:
                                        try:
                                            ...
                                            playlists['typeXml'] = typeXml
                                            open(PlaylistFile,'a').write(str(playlists))
                                            xbmc.executebuiltin("XBMC.Notification([B][COLOR=FF67cc33]mafiatv[/COLOR][/B],[B]Playlist XML Changed[/B],4000,"")")
                                        except: pass
                                    xbmc.executebuiltin("Container.Refresh")
                                ...
                                if title == urllib.unquote_plus(name) and urllib.unquote_plus(folders)==urllib.unquote_plus(folder):
                                    ...
                                    for name,url,fanart,folder,typeXml,thumb in playlist:
                                        try:
                                            ...
                                            playlists['typeXml'] = typeXml
                                            open(PlaylistFile,'a').write(str(playlists))
                                            xbmc.executebuiltin("XBMC.Notification([B][COLOR=FF67cc33]mafiatv[/COLOR][/B],[B]Playlist Thumbnail Changed[/B],4000,"")")
                                        except: pass
                                    xbmc.executebuiltin("Container.Refresh")
        if ret == 3:
            newfanart = xbmcgui.Dialog().browse(2, "[B][COLOR=FF67cc33]Fanart File Location[/COLOR][/B]", browseType)
