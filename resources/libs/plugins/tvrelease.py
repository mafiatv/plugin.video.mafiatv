...
def LISTHOSTERS(name,url):
    ...
    if len(r) == 0:
        addon.show_ok_dialog(['No Playable Streams Found,','It Might Be That They Are Still Being Uploaded,',
                              'Or They Are Unstreamable Archive Files'],'mafiatv: TV-Release')
    ...
    try:
        ...
        if '<h2>Under Maintenance</h2>' in h:
            addon.show_ok_dialog(['[COLOR=FF67cc33][B]TV-Release is Down For Maintenance,[/COLOR][/B]',
                                  '[COLOR=FF67cc33][B]Please Try Again Later[/COLOR][/B]',''],'mafiatv: TV-Release')
            xbmcplugin.endOfDirectory(int(sys.argv[1]), False, False)
            return
        return h
    except urllib2.URLError, e:
        addon.show_small_popup('mafiatv: Tv-Release','TV-Release Web Site Failed To Respond, Check Log For Details', 9000, error_logo)
