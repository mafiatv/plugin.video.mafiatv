...
def GENRES(url):
    ...
    try:
        ...
        if html == None:
            print 'html None'
            return
    except:#MafiaTV can you add your error calling function
...
def SEARCH(murl):
    if murl == 'tv':
        ...
        try:
            ...
            pass
            keyb = xbmc.Keyboard('', '[COLOR=FF67cc33]mafiatv: Search For Shows or Episodes[/COLOR]')
            keyb.doModal()
            if (keyb.isConfirmed()):
                    search = keyb.getText()
        ...
        try:
            ...
            pass
            keyb = xbmc.Keyboard('', '[COLOR=FF67cc33]mafiatv: Search For Movies[/COLOR]')
            keyb.doModal()
            if (keyb.isConfirmed()):
                    search = keyb.getText()
                    encode=urllib.quote(search)
