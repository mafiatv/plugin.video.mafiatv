...
try:
    ...
    elogo = xbmc.translatePath('special://home/addons/plugin.video.mafiaTV/resources/art/bigx.png')
    dialog = xbmcgui.Dialog()
    ok=dialog.ok('[B][COLOR=FF67cc33]MafiaTV Import Error[/COLOR][/B]','Failed To Import Needed Modules',str(e),'Report missing Module at [COLOR=FF67cc33]mafiatvxbmc.com[/COLOR] to Fix')
    xbmc.log('MafiaTV ERROR - Importing Modules: '+str(e), xbmc.LOGERROR)
...
def MAIN():
    ...
    for index, value in sorted(enumerate(d), key=lambda x:x[1]):
        ...
        if index==0:
            ...
            main.addDirHome("MafiaTV How To's",'how',16,art+'/howto.png')
        elif index==25:
            main.addDirHome('Fixes','http://www.movie25.ag/',784,art+'/fixes.png')
        elif index==26:
            ...
            main.addDirHome('ONE242415','https://raw.github.com/mafiatv/MafiaTVRepo/master/MafiaTVRepo_Directory.xml',235,art+'/one252515.png')
        elif index==30:
            main.addDirHome('My XML Channels','nills',238,art+'/xml.png')
        elif index==31:
            main.addDirHome("K1M05's Streams",'https://raw.github.com/crusader88/K1M05/master/k1m05_mafiatvDirectory.xml',181,art+'/k1m05.png')
        elif index==32:
            ...
            if selfAddon.getSetting("stracker") == '0':
                main.addDirHome("SideReel Show Tracker",'MafiaTV',397,art+'/sidereel.png')
            elif selfAddon.getSetting("stracker") == '1':
                main.addDirHome("Trakt Show Tracker",'MafiaTV',429,art+'/trakt.png')
            else:
                main.addDirHome("SideReel Show Tracker",'MafiaTV',397,art+'/sidereel.png')
                main.addDirHome("Trakt Show Tracker",'MafiaTV',429,art+'/trakt.png')
        elif index==38:
    ...
    main.addPlayc('Upload Log','http://www.movie25.ag/',156,art+'/loguploader.png','','','','','')
    main.addPlayc('Click Me!!!','https://raw.github.com/mafiatv/MafiaTVArtwork/master/skins/vector/donation.png',244,art+'/paypalmash2.png','','','','','')
    main.addSpecial('@mafiatvxbmc','','',art+'/twittermash.png')
    main.addPlayc('mafiatv Settings','http://www.movie25.ag/',1999,art+'/MashSettings.png','','','','','')
              
def Announcements():
    #Announcement Notifier from xml file
    try:
        import time
        link=main.OPENURL('https://raw.github.com/mafiatv/MafiaTVNotifications/master/Notifier.xml?'+ str(time.time()),verbose=False)
        ...
        if match:
            ...
            if new != ' ':
                ...
                if not os.path.exists(notified):
                    ...
                    TextBoxes("[B][COLOR red]MafiaTV Announcements[/B][/COLOR]",anounce)
                if old != ' ':
        ...
        if new != ' ':
            ...
            if not os.path.exists(notified):
                ...
                if '</mu>' in video:
                    ...
                    movieplaylist.MLink2('mafiatv',video,'',muvideo=True)
                else:
                    from resources.libs import youtube
                    youtube.YOULink('mafiatv',video,'')
            if old != ' ':
                notified=os.path.join(runonce,str(old))
                if  os.path.exists(notified):
                    os.remove(notified)
...
def CheckForAutoUpdate(force = False):
	if selfAddon.getSetting("autosource") == "false":
		...
		GitHubBranch  = 'master'
		RunningFile   = 'running'
		verCheck=False #main.CheckVersion()#Checks If Plugin Version is up to date
		if verCheck == True:
			from resources.libs import autoupdate
			try:
				print "mafiatv auto update - started"
			...
			if locver < gitver and (not os.path.exists(RunningFilePath) or os.stat(RunningFilePath).st_mtime + 120 < time.time()) or force:
				...
				xbmc.executebuiltin("XBMC.Notification(mafiatv Update,New Update detected,3000,"+main.slogo+")")
				xbmc.executebuiltin("XBMC.Notification(mafiatv Update,Updating...,3000,"+main.slogo+")")
				...
				if os.path.isfile(UpdateLocalFile):
					...
					if autoupdate.unzipAndMove(UpdateLocalFile,extractFolder,pluginsrc):
						...
						print "mafiatv auto update - update install successful ("+str(gitver)+")"
						xbmc.executebuiltin("XBMC.Notification(mafiatv Update,Successful,5000,"+main.slogo+")")
						...
						print "mafiatv auto update - update install failed ("+str(gitver)+")"
						xbmc.executebuiltin("XBMC.Notification(mafiatv Update,Failed,3000,"+main.elogo+")")
						main.GA("Autoupdate",str(gitver)+" Failed")
				else:
					print "mafiatv auto update - cannot find downloaded update ("+str(gitver)+")"
					xbmc.executebuiltin("XBMC.Notification(mafiatv Update,Failed,3000,"+main.elogo+")")
					main.GA("Autoupdate",str(gitver)+" Repo problem")
				try:os.remove(RunningFilePath)
				except:pass
			else:
				if force: xbmc.executebuiltin("XBMC.Notification(mafiatv Update,mafiatv is up-to-date,3000,"+main.slogo+")")
				print "mafiatv auto update - mafiatv is up-to-date ("+str(locver)+")"
			return
	else:
...
def Notify():
    ...
    notified=os.path.join(runonce,str(mafiatv))
    if not os.path.exists(notified):
        open(notified,'w').write('version="%s",'%mafiatv)
        dir = selfAddon.getAddonInfo('path')
        chlg = os.path.join(dir, 'changelog.txt')
        TextBoxes("[B][COLOR red]MafiaTV Changelog[/B][/COLOR]",chlg)
        mafiatv=mafiatv-1
        notified=os.path.join(runonce,str(mafiatv))
        if  os.path.exists(notified):
            os.remove(notified)
...
def GENRE(url,index=False):
    main.addDir('Action','http://www.movie25.ag/movies/action/',1,art+'/act.png',index=index)
    main.addDir('Adventure','http://www.movie25.ag/movies/adventure/',1,art+'/adv.png',index=index)
    main.addDir('Animation','http://www.movie25.ag/movies/animation/',1,art+'/ani.png',index=index)
    main.addDir('Biography','http://www.movie25.ag/movies/biography/',1,art+'/bio.png',index=index)
    main.addDir('Comedy','http://www.movie25.ag/movies/comedy/',1,art+'/com.png',index=index)
    main.addDir('Crime','http://www.movie25.ag/movies/crime/',1,art+'/cri.png',index=index)
    main.addDir('Documentary','http://www.movie25.ag/movies/documentary/',1,art+'/doc.png',index=index)
    main.addDir('Drama','http://www.movie25.ag/movies/drama/',1,art+'/dra.png',index=index)
    main.addDir('Family','http://www.movie25.ag/movies/family/',1,art+'/fam.png',index=index)
    main.addDir('Fantasy','http://www.movie25.ag/movies/fantasy/',1,art+'/fant.png',index=index)
    main.addDir('History','http://www.movie25.ag/movies/history/',1,art+'/his.png',index=index)
    main.addDir('Horror','http://www.movie25.ag/movies/horror/',1,art+'/hor.png',index=index)
    main.addDir('Music','http://www.movie25.ag/movies/music/',1,art+'/mus.png',index=index)
    main.addDir('Musical','http://www.movie25.ag/movies/musical/',1,art+'/mucl.png',index=index)
    main.addDir('Mystery','http://www.movie25.ag/movies/mystery/',1,art+'/mys.png',index=index)
    main.addDir('Romance','http://www.movie25.ag/movies/romance/',1,art+'/rom.png',index=index)
    main.addDir('Sci-Fi','http://www.movie25.ag/movies/sci-fi/',1,art+'/sci.png',index=index)
    main.addDir('Short','http://www.movie25.ag/movies/short/',1,art+'/sho.png',index=index)
    main.addDir('Sport','http://www.movie25.ag/movies/sport/',1,art+'/sport.png',index=index)
    main.addDir('Thriller','http://www.movie25.ag/movies/thriller/',1,art+'/thr.png',index=index)
    main.addDir('War','http://www.movie25.ag/movies/war/',1,art+'/war.png',index=index)
    main.addDir('Western','http://www.movie25.ag/movies/western/',1,art+'/west.png',index=index)
    main.GA("None","Movie25-Genre")
    main.VIEWSB()
        
def YEAR(index=False):
    main.addDir('2013','http://www.movie25.ag/search.php?year=2014/',8,art+'/2014.png',index=index)
    main.addDir('2013','http://www.movie25.ag/search.php?year=2013/',8,art+'/2013.png',index=index)
    main.addDir('2012','http://www.movie25.ag/search.php?year=2012/',8,art+'/2012.png',index=index)
    main.addDir('2011','http://www.movie25.ag/search.php?year=2011/',8,art+'/2011.png',index=index)
    main.addDir('2010','http://www.movie25.ag/search.php?year=2010/',8,art+'/2010.png',index=index)
    main.addDir('2009','http://www.movie25.ag/search.php?year=2009/',8,art+'/2009.png',index=index)
    main.addDir('2008','http://www.movie25.ag/search.php?year=2008/',8,art+'/2008.png',index=index)
    main.addDir('2007','http://www.movie25.ag/search.php?year=2007/',8,art+'/2007.png',index=index)
    main.addDir('2006','http://www.movie25.ag/search.php?year=2006/',8,art+'/2006.png',index=index)
    main.addDir('2005','http://www.movie25.ag/search.php?year=2005/',8,art+'/2005.png',index=index)
    main.addDir('2004','http://www.movie25.ag/search.php?year=2004/',8,art+'/2004.png',index=index)
    main.addDir('2003','http://www.movie25.ag/search.php?year=2003/',8,art+'/2003.png',index=index)
...
def GlobalFav():
    if selfAddon.getSetting("groupfavs") == "true":
        ...
        main.addLink("[COLOR red]MafiaTV Fav's can also be favorited under XBMC favorites[/COLOR]",'','')
        main.addDir("Downloaded Content",'MafiaTV',241,art+'/downloadlog.png')
        main.addDir("Movie Fav's",'http://www.movie25.ag/',641,art+'/fav.png')
        main.addDir("TV Show Fav's",'http://www.movie25.ag/',640,art+'/fav.png')
        main.addDir("TV Episode Fav's",'http://www.movie25.ag/',651,art+'/fav.png')
        main.addDir("Live Fav's",'http://www.movie25.ag/',648,art+'/fav.png')
        main.addDir("Misc. Fav's",'http://www.movie25.ag/',650,art+'/fav.png')
    
def TV():
    main.ClearDir(TempPath)
    if selfAddon.getSetting("stracker") == '0':
        main.addDir("SideReel Show Tracker",'MafiaTV',397,art+'/sidereel.png')
    elif selfAddon.getSetting("stracker") == '1':
        main.addDir("Trakt Show Tracker",'MafiaTV',429,art+'/trakt.png')
    else:
        main.addDir("SideReel Show Tracker",'MafiaTV',397,art+'/sidereel.png')
        main.addDir("Trakt Show Tracker",'MafiaTV',429,art+'/trakt.png')
...
def ThreeDsec():
    ...
    link=getListFile('https://raw.github.com/mafiatv/MafiaTVNotifications/master/Directories/3D_Directory.xml', os.path.join(CachePath,'ThreeD'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
...
def Movie25(index=False):
    main.addDirHome('Search','http://www.movie25.ag/',420,art+'/search2.png',index=index)
    main.addDirHome('A-Z','http://www.movie25.ag/',6,art+'/az2.png',index=index)
    main.addDirHome('New Releases','http://www.movie25.ag/movies/new-releases/',1,art+'/new2.png',index=index)
    main.addDirHome('Latest Added','http://www.movie25.ag/movies/latest-added/',1,art+'/latest2.png',index=index)
    main.addDirHome('Featured Movies','http://www.movie25.ag/movies/featured-movies/',1,art+'/feat2.png',index=index)
    main.addDirHome('Most Viewed','http://www.movie25.ag/movies/most-viewed/',1,art+'/view2.png',index=index)
    main.addDirHome('Most Voted','http://www.movie25.ag/movies/most-voted/',1,art+'/vote2.png',index=index)
    main.addDirHome('HD Releases','http://www.movie25.ag/movies/latest-hd-movies/',1,art+'/dvd2hd.png',index=index)
    main.addDirHome('Genre','http://www.movie25.ag/',2,art+'/genre2.png',index=index)
    main.addDirHome('By Year','http://www.movie25.ag/',7,art+'/year2.png',index=index)
...
def INTCAT(murl):
    if 'italian'in murl:
        main.addDir('Cinema Italiano','https://raw.github.com/mafiatv/MafiaTVRepo/master/CinemaItaliano/cinemaitaliano_directory.xml',236,art+'/intl.png')
        main.addDir('Italian Series','https://raw.github.com/mafiatv/MafiaTVRepo/master/Foriegn/italianseries.xml',236,art+'/intl.png')
        main.addDir('Live Italian TV','https://raw.github.com/mafiatv/MafiaTVRepo/master/Foriegn/italianLiveTV.xml',236,art+'/intl.png')
    if 'russian' in murl:
    ...
    if 'uk' in murl:
        ...
        main.addDir('Best of British (TNPB)','https://raw.github.com/mafiatv/MafiaTVTNPB/master/Best%20of%20British.xml',236,art+'/intl.png')
    if 'spanish' in murl:
    ...
    if 'danish' in murl:
        main.addDir('Staael1982 Danish Movies','https://raw.github.com/mafiatv/Staael1982/master/danish%20movies.xml',236,art+'/intl.png')
...
def SPORTS():
    ...
    link=getListFile('https://raw.github.com/mafiatv/MafiaTVNotifications/master/Sport_Directory.xml', os.path.join(CachePath,'Sports'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
    match=re.compile('<name>(.+?)</name><link>(.+?)</link><thumbnail>(.+?)</thumbnail><mode>(.+?)</mode>').findall(link)
    for name,url,thumb,mode in match:
        if re.findall('http',thumb):
...
def HOWTOCAT():
    main.addDir("mafiatv Installation Instructions",'PLzXXwZxGnHxbv-UVWa-TIoFUIXWCaY_rm',205,art+'/howto.png')
    main.addDir("mafiatv Tutorials",'PLzXXwZxGnHxaup4QuaJOk9lC5Oms9L6NL',205,art+'/howto.png')
    main.addDir("mafiatv Features & Reviews",'PLzXXwZxGnHxZS61kC2t0RdT-4ZcX44Ltd',205,art+'/howto.png')
...
def LiveStreams():
    ...
    link=getListFile('https://raw.github.com/mafiatv/MafiaTVNotifications/master/LiveDirectory(mash2k3Only).xml',os.path.join(CachePath,'LiveStreams'))
...
def DOCS():
    ...
    main.addDir('Documentaries (Movie25)','http://www.movie25.ag/movies/documentary/',1,art+'/doc.png')
    main.GA("None","Documentary")

def PlaylistDir():
    link=getListFile('https://raw.github.com/mafiatv/MafiaTVNotifications/master/MoviePlaylist_Dir.xml',os.path.join(CachePath,'Playlist'))
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
    match=re.compile('<name>(.+?)</name><link>(.+?)</link><thumbnail>(.+?)</thumbnail><mode>(.+?)</mode>').findall(link)
    for name,url,thumb,mode in match:
        if re.findall('http',thumb):
...
def MAINDEL(murl):
    if 'packages' in murl:
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('MafiaTV', 'Are you sure you want to delete packages folder','','','No', 'Yes')
        if ret:
            import os 
    ...
    if 'Malformed' in murl:
        xbmc.executebuiltin('XBMC.RunScript(special://home/addons/plugin.video.mafiaTV/resources/fixes/malformedFix.py)')
        ...
        dialog = xbmcgui.Dialog()
        if dialog.yesno('MafiaTV', 'Are you sure you want to clear XBMC cache ?','','','No', 'Yes'):
        ...
        dialog = xbmcgui.Dialog()
        if dialog.yesno('MafiaTV', 'Are you sure you want to clear mafiatv Cache & Cookies?','','','No', 'Yes'):
            import os
...
def DLLIBRTMP(mname,key,trigger):
    ...
    if re.search('(?i)linux',trigger):
        ...
        if (keyb.isConfirmed()):
            ...
            os.remove(lib)
    if re.search('APKINSTALLER',trigger):
        dialog.ok("MafiaTV", "Thats It All Done", "[COLOR blue]Download location[/COLOR]",path) 
    else:
        dialog.ok("MafiaTV", "Thats It All Done", "[COLOR blue]Now should be Updated[/COLOR]")
...
def MAINTENANCE(name):
    if name == 'MAINTENANCE':
        ...
        main.addSpecial('Clear mafiatv Cache & Cookies','MashCache',416,art+'/maintenance.png')
    else:
...
def FIXES():
    ...
    try:
        link=main.OPENURL('https://raw.github.com/mafiatv/MafiaTVFixes/master/Fixes.xml')
    except:
...
def FIXDOWN(name,filename,location,path):
    ...
    url = 'https://raw.github.com/mafiatv/MafiaTVFixes/master/FIXES/'+filename
    ...
    dialog.ok("MafiaTV", "Thats It All Done", "[COLOR blue]Now "+name+" should be Fixed[/COLOR]")

def ADSettings(name,filename):
    main.GA("Fixes",name+"-Fix")
    url = 'https://raw.github.com/mafiatv/MafiaTVFixes/master/AdvancedSettings/'+filename
    print "#############  Downloading from "+ url+"  #####################"
    ...
    dialog = xbmcgui.Dialog()
    name  = name.split('[COLOR red]')[0]
    dialog.ok("MafiaTV", "Thats It All Done", "[COLOR blue]Now "+name+" should be Installed[/COLOR]")

def delAS():
    dialog = xbmcgui.Dialog()
    if dialog.yesno('MafiaTV', 'Are you sure you want to remove advancedsettings.xml?','','','No', 'Yes'):
        path = xbmc.translatePath(os.path.join('special://home/','userdata'))
        file=os.path.join(path, 'advancedsettings.xml')
        os.remove(file)
        dialog.ok("MafiaTV", "Thats It All Done", "[COLOR blue]Now AdvancedSettings.xml is removed[/COLOR]")
...
def showLiveAnnouncements():
    #Announcement Notifier from xml file
    try:
        link=main.OPENURL('https://raw.github.com/mafiatv/MafiaTVNotifications/master/NotifierLive.xml', verbose=False)
    except: link='nill'
    link=link.replace('\r','').replace('\n','').replace('\t','').replace('&nbsp;','')
...
def AddToDixie(secName,name,murl,secIcon):
    ...
    if ret == -1:
        ...
        if not os.path.exists(SectionSource):
            ...
            open(SectionSource,'a').write("\n"+id+"="+path)
            xbmc.executebuiltin("XBMC.Notification(mafiatv,Successfully Added,2500,"+main.slogo+")")
        else:
            path = murl
            id = finalList[ret]
            open(SectionSource,'a').write("\n"+id+"="+path)
...
class HUBx( xbmcgui.WindowXMLDialog ):
    ...
    def _close_dialog( self ):
        path = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.mafiaTV/resources/skins/DefaultSkin','media'))
...
def getFavorites(section_title = None):
    ...
    if len(fav_items) > 0:
        ...
        for fav_item in fav_items:
            ...
            xbmc.executebuiltin("XBMC.Notification([B][COLOR=FF67cc33]MafiaTV[/COLOR][/B],[B]You Have No Saved Favourites[/B],5000,"")")
    return
...
def WHClear(url):
    dialog = xbmcgui.Dialog()
    ret = dialog.yesno('mafiatv Watch History', 'Are you sure you want to clear your','watch history, you can not restore','once you press yes','No', 'Yes')
    if ret:
        os.remove(url)
        xbmc.executebuiltin("XBMC.Container.Refresh")
...
def History():
    ...
    if selfAddon.getSetting("whistory") == "true":
        ...
        dialog = xbmcgui.Dialog()
        ok=dialog.ok('[B]MafiaTV History[/B]', 'Watch history is disabled' ,'To enable go to addon settings','and enable Watch History')
        history_items = wh.get_my_watch_history()
        for item in history_items:
            item_title = item['title']
            item_url = item['url']
...
#171 taken
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("MafiaTV", "Sorry this video requires a SkySports Suscription.","Will add this feature in later Version.","Enjoy the rest of the videos ;).")
